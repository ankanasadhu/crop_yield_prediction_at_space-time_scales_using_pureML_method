...
from test_LSTM_code import Test
import pandas as pd
...
class CodeRun:
    def __init__(self, args):
        ...
        if self.args.pred_time == "1_month":
            self.ftr_name = "ftr_names_TN_1_month.txt" # reading the feature names used for training the LSTM model 
            ...
            self.ftr_name = "ftr_names_TN_2_months.txt"
            self.ridge_models_path = self.args.load_models_path + 'Ridge_model_pureML_2months'
        else:
            self.ftr_name = "ftr_names_TN_15_days.txt"
            self.ridge_models_path = self.args.load_models_path + 'Ridge_model_pureML_15days'
        self.J_value = 8#Setting the iteration value for M-STAC (model hyperparameter)
    ...
    def train_neural_net(self):
        ...
        my_instance=NeuralNetworkTimeSeries(self.districts,[], self.train_start,
                                            self.train_end,
                                            self.val_start,
                                            self.val_end,
                                            self.train_years,
    ...
    def run_algo(self):
        ...
        if self.args.scale_data: # if the user wants to scale the data, then scale the training data
            district_data=pd.DataFrame(self.scale(df_train.reset_index(drop=True).iloc[:,2:]),columns=df_train.columns[2:])
        else:
            district_data = df_train
        modellstm = tf.keras.models.load_model(self.args.load_models_path + self.args.lstm_model_name) # load the trained LSTM model
        alpha_=200 # Regularization value
        # run the mstac algorithm
        my_instance=MSTAC(modellstm,[],
                          self.train_start,
                          self.train_end,
                          self.val_start,
                          self.val_end,
    ...
    def scale(self, data):
        ...
        scaler_path = f"../groundnut_models/scaler_{scaler_name}.pkl" # initialize a name for saving the scaler model based on the prediction time-horizon
