...
class Test:
    ...
    def __init__(self, filepath_lstm,folder_rrc,filepath_data_previous,num_districts,path, test_data, J_value, ftr_name, test_yield_path):
        self.filepath_lstm=filepath_lstm###saved lstm model
        self.folder_rrc=folder_rrc###saved mstac model
        ...
        self.test_yield = pd.read_csv(test_yield_path)
    ...
    def model_test(self):
        ...
        for j in range (J):
            ...
            model_kmeans=joblib.load( self.folder_rrc+'/'+str(j+1)+'_iteration_kmeans_model.pkl')
            ...
            for d in range (J-j):### Loading the models for d-th cluster
                polys.append(joblib.load(self.folder_rrc+'/'+str(j+1)+'_iteration_'+str(d+1)+'poly_transformer.pkl'))
                
                models_.append(joblib.load(self.folder_rrc+'/'+str(j+1)+'_iteration_'+str(d+1)+'cluster_ridge_model.pkl'))
        ...
        ## This code snippet generates a graphical representation of the prediction of test data.
        ## The graph is stored in the filepath
