...
class MSTAC :
    ...
    def prediction(self,a,degree,res,train_data,y_pred,data_fit):
        ...
        x_poly =polynomial_features.fit_transform(train_data)### defining polynomial features transformation model
    ...
    def model (self, progress_bar=None, status_label=None):
        ...
        ftr_path = "../soybean_data/" + self.ftr_name
        indx=ast.literal_eval(open(ftr_path, "r", encoding="utf-8").read()) ## reading feature names
        ...
        train_val_data=self.data[indx].iloc[self.train_start:self.val_end,:] 
        ...
        y_pred=self.modellstm(input_lstm)
        ...
        cluster_train_val_input=self.data[['nitrogen',  'phh2o', "Prediction"]].iloc[self.train_start:self.val_end,:]
        ...
        for j in range(J):
            ...
            for d in range (J-j):
                ...
                training_validation_data=train_val_data[cluster_train_val==d]
                res_data_train_val=res[cluster_train_val==d]
                
                validation_data=val_data[cluster_val==d]
                res_val=res[self.val_start:self.val_end]
                res_data_val=res_val[cluster_val==d]
                # results = permutation_importance(model,poly.transform(training_validation_data) ,res_data_train_val, scoring='neg_mean_squared_error')
                if(res_data_val.shape[0]!=0):
                    results = permutation_importance(model,poly.transform(validation_data) ,res_data_val, scoring='neg_mean_squared_error')
                    weighted_score=weighted_score+results.importances_mean*validation_data.shape[0]/(self.val_end-self.val_start)
                # print(training_validation_data.shape[0],(self.val_end-self.train_start))
               
                # weighted_score=weighted_score+results.importances_mean*training_validation_data.shape[0]/(self.val_end-self.train_start)
            ...
            df=pd.DataFrame(weighted_score.reshape(1,len(names)),columns=names )
            
            
            
            scores=pd.concat([scores,df],axis=0,ignore_index=True )
            # print(scores.shape)
            ...
            nmse_train=(self.nmse(y_train.values.reshape(self.train_years*self.num_districts),train_pred.reshape(self.train_years*self.num_districts))) ###Calculating sMAPE for training set          
            ...
            # scores.to_csv(self.filepath+'/'+'model_scores_new_val.csv', index=True)
