...
class NeuralNetworkTimeSeries:
    ...
    def model(self):
        ...
        def train_model() :
            ## model training
            model.fit(train_data_, y_train_, epochs=1000, batch_size=self.num_districts, shuffle=True,validation_data=(val_data_,y_val_), callbacks=[early_stopping, ProgressBarCallback()]
            )
